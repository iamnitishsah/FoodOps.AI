...
- **Target Transformation:** $\log(1 + \text{num\_orders})$ to stabilize right-skewed demand variance and heavy-tail spikes. Predictions are inverted back to raw order counts via $\max(0, \exp(\hat{y}_{\log}) - 1)$.
